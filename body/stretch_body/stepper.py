...
class Stepper(Device):
    ...
    def pretty_print(self):
        ...
        print('Waypoint Trajectory Target (rad) {} (deg) {}'.format(self.status['pos_traj'],
                                                                    rad_to_deg(self.status['pos_traj'])))
    ...
    def push_waypoint_trajectory(self, next_segment):
    ...
    def pack_traj_seg(self, s, sidx):
        with self.lock:
            for i in range(7):
                pack_float_t(s, sidx, self.traj_next_seg[i])
                sidx += 4
            pack_uint8_t(s, sidx, self.traj_next_seg[7])
            sidx += 1
            return sidx
